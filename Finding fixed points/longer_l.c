...
//This program makes a list of orbits of the action of the group on bases. It starts from the output of the code q-short_l.c.
...
int ispossible(char *l, int z)			//checks for the list of signs of tests of length z whether it can be extended to a full list of signs in whole_l.c. Most sign combination do not lead to a chirotope and that is what we want to exclude here. 
{
	int i,j,k,p,q,limit;

	for (i=0;i<z;i++)			//check only for the orbits that have not been checked yet
		if (l[i])
			for (j=0;j<sizeofgroup;j++)
			{				
				limit=sizeofgroup;
...
void makepossible(char *l,int i,int z)			//z is the length of l that we are making, in final version z=testnr
							//make first z signs of tests that do not lead to a contradiction
{
	if (i==z)
	{
		if (ispossible(l,z))
		{
			...
			sprintf(text,"q-possible_chirotopes_tests%d_%d_Z%d_length%d.txt",R,N,sizeofgroup,new_size);
...
main()
{
	...
	printf("N=%d, R=%d, sizeofgroup=%d, old size=%d, new size=%d, %.2f min, %llu l's\n",N,R,sizeofgroup,size,new_size,((double)(end-begin))/CLOCKS_PER_SEC/60,made);
