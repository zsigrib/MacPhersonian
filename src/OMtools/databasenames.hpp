...
std::string OM_set(int, int) {
    static_assert(false, "There is no OM database for the specified parameters!");
    return "";
}

template<>
inline std::string OM_set<3,6>(int n_bases, int idx) {
    if (idx != 0) return "";
    return std::format("../../../resources/oriented_matroid_sets/r3n6/OMs_rank3_6elements_{}bases.txt", n_bases);
}

template<>
inline std::string OM_set<3,7>(int n_bases, int idx) {
    if (n_bases < 27) {
        if (idx != 0) return "";
